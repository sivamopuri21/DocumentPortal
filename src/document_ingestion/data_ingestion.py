...
class FaissManager:
    ...
    def _save_meta(self):
        self.math_path.write_text(json.dumps(self._meta,ensure_ascii=False, indent=2), encoding="utf-8")

    def load_or_create(self,texts:Optional[List[str]]=None, metadatas: Optional[List[dict]] = None):
        if self._exists():
            self.vs = FAISS.load_local(str(self.index_dir), self.emb, allow_dangerous_deserialization = True)
        if not texts:
            raise DocumentPoratalException("No existing FAISS index and no data to create one", sys)
    ...
    def add_documents(self,docs: List[Document]):
        if self.vs is None:
            raise RuntimeError("Call load_or_create() before add_documents_idempodent()")
...
class ChatIngestor:
    def __init__( self,
        ...
        try:
            ...
            raise DocumentPoratalException("Initialization error in ChatIngestor", e) from e
    ...
    def built_retriver( self,
        ...
        *,
        ...
        try:
            ...
            raise DocumentPoratalException("Failed to build retriever", e) from e
...
class DocHandler:
    ...
    def save_pdf(self, uploaded_file) -> str:
        try:
            ...
            raise DocumentPoratalException(f"Failed to save PDF: {str(e)}", e) from e
    ...
    def read_pdf(self, pdf_path: str) -> str:
        try:
            ...
            raise DocumentPoratalException(f"Could not process PDF: {pdf_path}", e) from e
class DocumentComparator:
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        try:
            ...
            raise DocumentPoratalException("Error saving files", e) from e
    ...
    def read_pdf(self, pdf_path: Path) -> str:
        try:
            ...
            raise DocumentPoratalException("Error reading PDF", e) from e
    ...
    def combine_documents(self) -> str:
        try:
            ...
            raise DocumentPoratalException("Error combining documents", e) from e
    ...
    def clean_old_sessions(self, keep_latest: int = 3):
        try:
            ...
            raise DocumentPoratalException("Error cleaning old sessions", e) from e
